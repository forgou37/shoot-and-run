...
- **Host migration:** when a player-host quits it is genuinely hard; v1 policy is "host leaving ends the session." Dedicated mode sidesteps it — there is no peer to migrate authority to. **Moved to 012** (it is only meaningful with a player-host); see specs/013-netplay-polish.md § reorder.
- **Lag compensation:** **resolved in 013** (specs/013-netplay-polish.md, OA). In the deterministic host-authoritative rollback model shipped in 009–011 every machine simulates the same tick with the same committed inputs, so "the shooter's view" and "the host's view" converge by construction — there is no per-client divergence to rewind. Source-style shooter-favored server hit-rewind is therefore **rejected** (it would make canonical state depend on per-client views and break determinism). "Lag-comp tuning" = **input-delay strategy + rollback-correction smoothing**, all as `net`-block data. (This corrects the earlier exploratory "default modest shooter-favor" note.)
...
| 012 | Player-hosted / listen-server: one browser becomes the Host; WebRTC DataChannel P2P; NAT traversal + TURN; host-leaving policy + **host migration** (moved from 013 — listen-server-only concept) | yes (P2P) | 011 |
| **013** | Polish (dedicated-server): spectators, reconnection, metrics/telemetry, anti-cheat posture, lag-comp tuning — **done** (T13.1–T13.6), own spec (specs/013-netplay-polish.md). **Reordered ahead of 012**: these five ride the existing 011 dedicated host and need no player-hosting; host migration split out to 012. | — | **011 ✓** |
