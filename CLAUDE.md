...
| `content/tuning.json` | Every game-feel number. The only place tunables exist. Shell-only blocks `juice`, `input` (stickDeadzone), `ui` (lobbyCountdownMs), and `net` (inputDelayTicks/snapshotIntervalTicks/maxRollbackTicks/jitterBufferTicks, spec 008/009, validated in `packages/net`) live here too — the sim ignores them all. |
...
│  ├─ 000-baseline.md
│  └─ backlog.md
├─ assets/                    # Aseprite sprite sources (art pass, spec 006); card_*.aseprite (lobby character cards); fonts/ (FreePixel.ttf, the shell's pixel font)
├─ scripts/
...
│  ├─ net/                    # spec 008: host-authoritative rollback netcode (pure, headless, imports only sim). types: transport/protocol/session; impl (Phase 009): loopback (latency/jitter/loss), codec (NetMessage wire), host (authoritative loop), clock (tick sync), rollback (predict+rollback), params (tuning.net validator). Real transport/browser in 010+
│  └─ game/
│     ├─ index.html
│     ├─ vite.config.ts
│     ├─ public/assets/       # committed sprite atlases (PNG + Aseprite JSON), via export:art; card_*.png (96×180 lobby cards, plain images)
│     └─ src/
...
│        └─ render/           # sprite renderers (archer, arrows, jungle env); cards.ts (lobby card art loader); rect debug via ?rects=1
...
| `npm run typecheck` | `tsc --noEmit` over sim src + tests, bots src + tests (no Node/DOM types — purity), net src, game, and e2e |
