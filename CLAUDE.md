...
Long-term direction (specs 003–004, **not now**): an offline AI pipeline generates arena layouts as JSON, a headless simulator plays them with scripted bots, balance metrics + LLM-as-judge score them, winners land in `content/arenas/`. The architecture below exists so this is possible without a rewrite.
...
| 2026-06-12 | sim-events | player_killed carries the victim's position (x, y) | shell FX (kill burst placement) need it now; spec-004 balance metrics need kill heatmaps later; events are the sim's only output channel, so position must be in the payload | shell reading victim position from state rejected: at the time the event is consumed the round may already have reset; golden log consciously regenerated once for the payload change |
